...
**当示例代码非常长或是完整代码原文时，使用文件引用而非内联代码**：

- **使用路径引用**：格式为 \`文件路径:行号范围\`，例如：
  - \`agents/code/graph.ts:45-60\`
  - \`tui/src/app.tsx\`

- **引用说明**：说明该部分代码的功能或关键点，例如：
  - "参见 \`agents/code/middlewares/subagents.ts:28-50\`：SubAgent 状态管理实现"
  - "完整实现参考 \`tui/src/chat/store/index.ts:150-200\`：数据库持久化逻辑"
...
- **长代码处理方式**：
  ...
  推荐：
  ## 代码示例
  核心逻辑参见 \`agents/code/graph.ts:120-180\`：中间件链执行顺序
