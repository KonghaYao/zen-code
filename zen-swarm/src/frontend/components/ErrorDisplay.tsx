...
export function ErrorDisplay(props: ErrorDisplayProps) {
    ...
    return (
        <div className="bg-white border border-border-subtle rounded-lg p-6 text-center shadow-sm">
            <div className="text-error mb-3 flex items-center justify-center gap-2">
                <span className="text-2xl">⚠️</span>
                <span className="text-lg font-medium">Error</span>
            </div>
            <div className="text-text-secondary text-sm mb-4">{errorMessage}</div>
            {props.onRetry && (
...
export function EmptyState(props: { message: string; action?: { label: string; onClick: () => void } }) {
    return (
        <div className="bg-white border border-border-subtle rounded-lg p-8 text-center text-text-muted">
            ...
            <p className="text-text-secondary">{props.message}</p>
            {props.action && (
