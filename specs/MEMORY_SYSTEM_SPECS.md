...
Memory System 是一个类似于 Skills System 的渐进式披露系统，用于存储和检索 AI Agent 在任务执行过程中获得的知识和经验。与 Skills 系统类似，Memory 也通过 YAML frontmatter + Markdown 文件的形式组织，并通过 Middleware 自动加载到系统提示中。
...
\`\`\`typescript
export const initChatModel = async (mainModel: string, config?: {}) => {
    let model;
...
export class MemoriesMiddleware implements AgentMiddleware {
    ...
    async wrapModelCall(request: any, handler: any): Promise<AIMessage> {
        ...
        // 注入到系统提示
        const memoriesSection = MEMORIES_SYSTEM_PROMPT
...
## 理由
1. pnpm 更节省磁盘空间（使用硬链接）
2. pnpm 的安装速度更快
3. pnpm workspace 配置更简洁
4. 项目已经在使用 pnpm 作为包管理器
...
const schema = z.object({
    name: z.string(),
    email: z.string().optional(),  // NEW: 添加 optional
    phone: z.string().optional(),  // NEW: 添加 optional
});
\`\`\`
...
✅ **渐进式披露** - 只加载元数据，完整内容按需读取
✅ **版本控制友好** - 纯文本文件，易于版本管理
✅ **简洁设计** - 单个 MEMORY.md 文件，无需复杂目录结构
✅ **中文友好** - 所有内容使用中文，便于理解和维护
