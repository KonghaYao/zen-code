...
将 MCP Middleware 从中间件架构改造为独立单例服务，提供统一的工具管理和缓存机制。MCP 工具通过 LangChain tool 接口暴露，与 Command System 完全解耦。
...
export async function MCPMiddleware(options?: ClientConfig['mcpServers']) {
  ...
  return createMiddleware({
    ...
    afterAgent() { client.close(); }
  });
}

// graph.ts
const graph = await createGraph(config, [
  { middleware: await MCPMiddleware(config.mcp_config) }
]);
```
