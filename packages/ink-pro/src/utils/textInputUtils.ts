...
export function getCharWidth(char: string): number {
    ...
    if ((code >= 0xff01 && code <= 0xff60) || (code >= 0xffe0 && code <= 0xffe6)) {
        return 2;
    }
 ...
 */
export function lineColumnToOffset(lines: string[], line: number, column: number): number {
    if (lines.length === 0) {
        return 0;
    }
...
export function clampCursor(
    ...
    // Clamp column to valid range for this line
    const clampedColumn = Math.max(0, Math.min(cursorColumn, lines[clampedLine].length));
