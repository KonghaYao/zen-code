...
export const ApprovalProvider: React.FC<ApprovalProviderProps> = ({ children }) => {
    ...
    const addApprovalRequest = useCallback(
        (request: Omit<ApprovalRequest, 'id' | 'createdAt' | 'status'>) => {
            ...
            setRequests((prev) => {
                ...
                // 超过最大数量限制时，移除最旧的已处理请求
                if (updated.length > MAX_REQUESTS) {
                    const completed = updated.filter((req) => req.status !== ApprovalStatus.Pending);
                    if (completed.length > 0) {
                        return updated.filter((req) => req.id !== completed[0].id);
                    }
                }
     ...
     * 移除审批请求
     */
    const removeApprovalRequest = useCallback(
        (id: string) => {
            setRequests((prev) => {
    ...
    // 计算是否有待处理的审批请求
    const hasPendingRequests = useMemo(() => requests.some((req) => req.status === ApprovalStatus.Pending), [requests]);
    ...
    const cleanupExpiredRequests = useCallback(() => {
        const now = Date.now();
        setRequests((prev) => prev.filter((req) => req.createdAt.getTime() > now - REQUEST_TIMEOUT));
    }, []);

    // ========== 自动清理 ==========

    // 使用 usehooks-ts 的 useInterval 定期清理超时请求
    useInterval(() => {
