 ...
 */
export type Decision = ApproveDecision | EditDecision | RejectDecision | RespondDecision;
...
const contextSchema = z.object({
     ...
     */
    interruptOn: z.record(z.string(), z.union([z.boolean(), InterruptOnConfigSchema])).optional(),
    /**
 ...
 * @public
 */
export function humanInTheLoopMiddleware(options: NonNullable<HumanInTheLoopMiddlewareConfig>) {
    ...
    return createMiddleware({
        ...
        afterModel: {
            ...
            hook: async (state, runtime) => {
                 ...
                 */
                const lastMessage = [...messages].reverse().find((msg) => AIMessage.isInstance(msg)) as
                    | BaseMessage
                    | undefined;
                ...
                const hasHandledToolCalls = decisions.some(
                    (decision) => decision.type === 'reject' || decision.type === 'respond',
                );
                ...
                for (let i = 0; i < decisions.length; i++) {
                    ...
                    if (
                         ...
                         */
                        (!hasHandledToolCalls || decision.type === 'reject' || decision.type === 'respond')
                    ) {
                        revisedToolCalls.push(revisedToolCall);
                    }
                    if (toolMessage) {
                        artificialToolMessages.push(toolMessage as unknown as BaseMessage);
                    }
                }
                ...
                const jumpTo: JumpToTarget | undefined = hasHandledToolCalls ? 'model' : undefined;
                return {
                    messages: [lastMessage as BaseMessage, ...artificialToolMessages],
                    jumpTo,
                };
            },
        },
