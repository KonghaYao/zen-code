...
export const storeRouter = router({
    ...
    listStores: publicProcedure.query(({ ctx }) => {
        return ctx.remoteStoreStorage.list().map((s) => ({
            id: s.id,
            name: s.name,
            type: s.type,
            base_url: s.base_url,
    ...
    addStore: publicProcedure.input(RemoteStoreInputSchema).mutation(({ ctx, input }) => {
        const id = input.id ?? crypto.randomUUID();
        const type = input.type ?? 'generic_http';
        const base_url = input.base_url ?? (type === 'clawhub' ? CLAWHUB_BASE_URL : '');
        return ctx.remoteStoreStorage.insert({
            ...input,
            id,
            type,
            base_url,
    ...
    updateStore: publicProcedure.input(RemoteStoreUpdateSchema).mutation(({ ctx, input }) => {
        const result = ctx.remoteStoreStorage.update(input);
        if (!result) handleNotFound('RemoteStore', input.id);
        return result;
    }),

    deleteStore: publicProcedure.input(z.object({ id: z.string() })).mutation(({ ctx, input }) => {
        const ok = ctx.remoteStoreStorage.delete(input.id);
    ...
    listRemotePrompts: publicProcedure
        ...
        .query(async ({ ctx, input }) => {
            const entry = ctx.remoteStoreStorage.get(input.storeId);
